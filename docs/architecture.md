...
flowchart LR
    ...
    API[axum: bin/api] --> Domain[Домены: auth, users, estimates]
...
server/src/
  core/        инфраструктура, общая для доменов:
    ...
    llm.rs       вызов нейросети (OpenAI-совместимый API), учёт и потолок
  ...
  jobs.rs      фоновый воркер приложения (письма, уборка, разбор смет):
               лежит рядом с core, потому что знает о доменах, а core — нет
...
- Права проверяются В ЗАПРОСЕ, а не отдельным `if` после выборки: доменные
  функции estimates принимают владельца и всегда фильтруют по нему. Чужая и
  несуществующая смета отвечают одинаково (404) — по ответу нельзя узнать,
  что такая смета вообще есть.
...
- Фоновых циклов ДВА (`jobs.rs`, оба живут в процессе api): почта с уборкой
  и разбор смет. Разделены намеренно: разбор фотографии — вызов нейросети
  длиной в минуту, и в общем цикле письмо сброса пароля ждало бы её. По той
  же причине кривой `SMTP_FROM` ломает только доставку писем, а не разбор.
- Почтовый цикл забирает неотправленные письма пачками через
  `FOR UPDATE SKIP LOCKED` и шлёт по SMTP (lettre). `SMTP_URL` не задан —
  письма остаются в outbox: так работают dev и тесты. После 5 неудач письмо
  больше не берётся.
- Он же раз в час зовёт `auth::cleanup_expired_tokens` — протухшие
  ...
  и выбрасывает старые счётчики вызовов нейросети. Поэтому воркер лежит не
  в `core`: `core` о доменах не знает, а планировщик знает по определению.
- Второй цикл разбирает загруженные сметы: сам цикл разбора живёт в домене
...
- Один модуль `core/llm.rs` и никакой абстракции «провайдер»: сам протокол
  `/chat/completions` (OpenAI-совместимый) и есть интерфейс, а смена
  провайдера или модели — три переменные окружения (`docs/llm.md`).
  Провайдероспецифичных полей не используем, иначе переезд перестанет быть
  бесплатным.
- Предохранители на месте вызова: нет ключа — интеграция выключена целиком;
  дневной потолок токенов на весь сервис (константа, UTC-сутки); учёт в
  таблице `llm_calls` — только счётчики, ни запросов, ни ответов.
- В `/ready` нейросеть НЕ входит: её недоступность не делает сервис
  неработоспособным — фото просто ждут в очереди, Excel разбирается как
  обычно.
- Ошибки различаются по одному признаку: виноват вход или нет. Провайдер
  недоступен, ключ неверен, потолок выбран — попытка разбора не тратится,
  смета ждёт следующего захода воркера.
