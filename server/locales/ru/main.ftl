...
error-estimate-format = Принимаем Excel (xlsx, xls) и фотографию сметы (jpg, png, webp)
error-estimate-photo-off = Разбор фотографий сейчас недоступен — пришлите смету файлом Excel
error-estimate-photo-needs-email = Чтобы прислать фото, подтвердите адрес почты — мы отправили вам письмо со ссылкой
error-estimate-not-a-photo = Файл не похож на фотографию — сфотографируйте смету заново
...
error-estimate-photo-unreadable = Не получилось прочитать смету на фотографии — переснимите её при хорошем свете, чтобы в кадр попали все строки
error-estimate-later = Пока не получилось разобрать смету — попробуйте загрузить её ещё раз позже
